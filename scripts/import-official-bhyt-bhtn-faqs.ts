/**
 * Import FAQ BHYT/BHTN từ nguồn chính thống (file curated) vào PostgreSQL.
 *
 *   pnpm run faq:import-official-bhyt-bhtn
 */
import "dotenv/config";
import { randomUUID } from "node:crypto";
import { Pool } from "pg";
import { OFFICIAL_BHYT_BHTN_FAQS } from "../src/lib/data/official-bhyt-bhtn-faqs";
...
async function main() {
  ...
  try {
    ...
    for (const f of OFFICIAL_BHYT_BHTN_FAQS) {
      ...
      const ins = await client.query<{ id: string }>(
        ...
        returning id
        `,
        [f.question, f.answer, f.answer, cat.rows[0].id, randomUUID()],
      );
    ...
    console.log(
      `Official BHYT/BHTN FAQ import: inserted=${inserted}, skipped=${skipped}`,
    );
  } catch (e) {
